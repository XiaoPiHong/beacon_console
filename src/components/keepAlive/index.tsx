...
import { JSXElementConstructor, memo, ReactElement, RefObject, useEffect, useRef, useState, createContext } from "react";
...
function KeepAlive({ children, exclude, include, maxLen = 5 }: Props) {
	...
	const isActive = components.current.findIndex(item => item.name === pathname);
	...
	if (isNil(exclude) && isNil(include)) {
		...
		const component = components.current.find(res => isEqual(res.name, pathname));
		if (isNil(component)) {
			components.current = [
				...components.current,
				{
		...
		<>
			...
			<KeepAliveContext.Provider value={context}>
				{components.current.map(({ name, ele }) => (
					<Component active={isEqual(name, pathname)} renderDiv={containerRef} name={name} key={name}>
						{ele}
					</Component>
				))}
